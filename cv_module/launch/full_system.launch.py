...
def generate_launch_description():
    ...
    # This rotates the camera frame so X=horizontal, Y=vertical, Z=depth
    # Parent: camera_color_optical_frame (actual camera frame)
    # Child:  camera_horizontal_frame (rotated visual frame)
    # Rotation: -90deg around Z axis + 180deg around Z axis to flip upside down
    static_tf = Node(
        package='tf2_ros',
    ...
    # Note: RealSense camera already provides rectified images at /camera/color/image_rect
    # No need for image_proc node

    # apriltag_ros detector - using raw images
    apriltag = Node(
        ...
        output='screen',
        parameters=[
            LaunchConfiguration('apriltag_settings_yaml'), 
            LaunchConfiguration('tags_yaml'),
            {'image_transport': 'raw'},
            {'publish_tf': True},  # Enable TF publishing for 6D poses
        ],
        remappings=[
            ('image_rect', '/camera/color/image_raw'),  # Use raw images
            ('camera_info', '/camera/color/camera_info'),
            ('detections', '/camera/tags'),  # Publish to apriltag_draw expected topic
        ]
    )
    ...
    return LaunchDescription([
        ...
        intrinsics_yaml, tags_yaml, apriltag_settings_yaml,
        realsense, static_tf, cam_override, apriltag, draw, bundle, rviz, frustum, tag_coords, rqt
    ])
